...
CRITICAL: You must ALWAYS propose actions for confirmation — never execute directly. Even if the Crawler previously confirmed a similar action, each new command requires a fresh CONFIRM. Never say "ACTION LOG: ✓" without a preceding confirmation in THIS message.
...
function renderScoreboard(){
  ...
  if(!el.dataset.built){
    el.dataset.built='1';
    document.documentElement.style.setProperty('--ui-scoreboard-panel', `url(${UI_SCOREBOARD_PANEL})`);
...
function buildWeekData(){
  ...
  return{
    ...
    date_range:fmtDonutDateRange(),
    current_day:new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'}),
    current_time:new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',hour12:true}),
    days,
    streak:calcStreak(),
    recovery_mode_count:0,
    floor_collapse_count:collapseState?.active?1:0,
    top_debuff:getTopDebuffName()
};}
