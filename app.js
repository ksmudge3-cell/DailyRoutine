...
function renderProfile(){
  ...
  const ednaFace=dogPct===100?CHAR_EDNA_HAPPY:dogPct>=50?CHAR_EDNA_GUARD:CHAR_EDNA_SIDE_EYE;
  const kronkFace=dogPct===100?CHAR_KRONK_EXCITED:dogPct>=50?CHAR_KRONK_HAPPY:CHAR_KRONK_FOOD;
...
async function generateWeeklySummary(force=false){
  ...
  try{
    const resp=await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({model:'claude-sonnet-4-5',max_tokens:1000,system:DONUT_SYSTEM_SUMMARY,messages:[{role:'user',content:`Here is the Crawler's weekly data:\n${JSON.stringify(weekData,null,2)}\n\nGenerate the weekly floor report.`}]})
    });
...
async function sendDonutMessage(message){
  ...
  try{
    ...
    const resp=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
      headers:{'Content-Type':'application/json','x-api-key':donutApiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},
      body:JSON.stringify({model:'claude-sonnet-4-5',max_tokens:1000,system:DONUT_SYSTEM_CHAT+`\n\nCurrent week data:\n${JSON.stringify(weekData,null,2)}`,messages:history})
    });
    const data=await resp.json();
